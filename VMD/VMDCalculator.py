...
class VMDCalculator():
    def __init__(self, Kmax=10, ALPHA=150):
        self.Kmax = Kmax
        # VMD的各项参数
        self.ALPHA = ALPHA
        self.TAU = 0.00001
    ...
    def myVMD(self, data, fs, K):
        ...
        '''
        u, u_hat, omega = VMD(f=data, alpha=self.ALPHA, tau=self.TAU, K=K, DC=self.DC, init=self.INIT, tol=self.TOL)
        # 只留下最终的omega
        omega = omega[-1, :] * fs
        # 计算各IMF与原始信号的相干性
        corr = []
    ...
    def findBestK(self, data, fs):
        ...
        for K in range(2, self.Kmax):
            u, omega, corr, IOvalue = self.myVMD(data, fs, K)
    ...
    def calculate(self, ts, fs=13.3333):
        K = self.findBestK(data=ts, fs=fs)
        if K > 0:
            u, omega, corr, IO = self.myVMD(data=ts, fs=fs, K=K)
            u = u[np.argmin(omega), :]
        return u